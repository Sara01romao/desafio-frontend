...
export function Input({type, name, label, value, onChange, ...props}:InputProps){
  return(
    <div className="relative w-full">
      <input type={type} name={name}  value={value} onChange={onChange} {...props} className="relative z-10 rounded-md peer w-full border-1 border-[#D8D8D8] placeholder:text-transparent p-2 focus:outline-[#007BFF] text-base pt-4 pb-1" placeholder="name" />
